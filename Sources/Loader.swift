...
public protocol Loading {
    ...
    func loadImage(with request: Request, token: CancellationToken?, completion: @escaping (Result<NukeImage>) -> Void)
}

public extension Loading {
    /// Loads an image with the given request.
    public func loadImage(with request: Request, completion: @escaping (Result<NukeImage>) -> Void) {
        loadImage(with: request, token: nil, completion: completion)
    }

    /// Loads an image with the given url.
    public func loadImage(with url: URL, token: CancellationToken? = nil, completion: @escaping (Result<NukeImage>) -> Void) {
...
public final class Loader: Loading {
    ...
    public var makeProcessor: (NukeImage, Request) -> AnyProcessor? = {
        return $1.processor
    }
    ...
    private func process(image: NukeImage, context ctx: Context) {
        queue.async {
    ...
    private struct Context {
        ...
        let completion: (Result<NukeImage>) -> Void
    }
